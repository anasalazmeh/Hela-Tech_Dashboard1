...
const AboutUsDetails = () => {
...

    {
      ...
      render: (_: any, record) => (
        <Space>
          ...
          <DeleteBtn
            onConfirm={async () => {
              await actions.deleteAboutUsDetails(record?.id);
            }}
            loading={loading.includes("delete")}
          />
        </Space>
  ...
  const { actions, list, loading } = useContext(AboutUsDetailsContext);
  ...
  return (
    <>
      <PageHeader
        title={"About Us Details Items"}
        subTitle={"All registered about us details items"}
        extra={[
          <Button
            type="primary"
            key={2}
            onClick={() => {
              navigate(`/about-us-details/create`);
            }}
            icon={<PlusOutlined />}
          >
            Add
...
export default AboutUsDetailsMainPage;
