...
const AboutUs = () => {
...

    {
      ...
      render: (_: any, record) => (
        <Space>
          ...
          <DeleteBtn
            onConfirm={async () => {
              await actions.deleteAboutUs(record?.id);
            }}
            loading={loading.includes("delete")}
          />
        </Space>
  ...
  const { actions, list, loading } = useContext(AboutUsContext);
  ...
  return (
    <>
      <PageHeader
        ...
        subTitle={"All registered about us items"}
        extra={[
          <Button
            type="primary"
            key={2}
            onClick={() => {
              navigate(`/about-us/create`);
            }}
            icon={<PlusOutlined />}
          >
            Add
